...
namespace MusicProject.Services.Identity
{
    ...
    //  validatePhoneNumer(user,errors);
      ...
      private void validatePhoneNumer(User user, List<IdentityError> errors)
      {
        ...
        if (string.IsNullOrWhiteSpace(userPhone))
        {
          if (string.IsNullOrWhiteSpace(userPhone))
          {
            ...
            {
              Code = "PhoneIsNotSet",
              Description = "لطفا اطلاعات همراه را تکمیل کنید."
            });
          }
          return; // base.ValidateAsync() will cover this case
        }

        if (!userPhone.IsValidIranianMobileNumber())
        {
        if (string.IsNullOrWhiteSpace(userPhone))
        {
          errors.Add(new IdentityError
          {
            Code = "PhoneIsNotSet",
            Description = "لطفا شماره همراه معتبری را وارد کنید."
          });
        }
      }
